// EnvTimeSimpleDebug.cs
// EnvTimeSimple 项目的统一日志封装，支持通过宏定义控制日志输出
//#define ENVTIMESIMPLE_ENABLE_DEBUG // 默认关闭
using UnityEngine;
...
{
    /// <summary>
    /// EnvTimeSimple 项目的统一日志调试类
    /// 
    /// 使用方法:
    /// 1. 在项目 Build Settings 中添加自定义宏定义 "ENVTIMESIMPLE_ENABLE_DEBUG" 来启用日志
    /// 2. 或在代码顶部使用 #define ENVTIMESIMPLE_ENABLE_DEBUG 局部启用
    /// 3. 未定义宏时，所有日志调用会被编译器优化移除（零性能开销）
    /// </summary>
    public static class EnvTimeSimpleDebug
    {
#if ENVTIMESIMPLE_ENABLE_DEBUG
        /// <summary>
        /// 启用/禁用日志输出的运行时开关（需 ENVTIMESIMPLE_ENABLE_DEBUG 宏已定义）
        /// </summary>
        ...
        /// 日志前缀
        /// </summary>
        private const string LOG_PREFIX = "[EnvTimeSimple]";
        /// <summary>
        /// 记录普通日志
        /// </summary>
        [System.Diagnostics.Conditional("ENVTIMESIMPLE_ENABLE_DEBUG")]
